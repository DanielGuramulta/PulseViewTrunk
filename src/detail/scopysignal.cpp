...
#include "pv/data/ScopySignalbase.hpp"
...
ScopySignal::ScopySignal(pv::Session &session,
    shared_ptr<data::ScopySignalBase> channel) :
	Trace(channel),
	session_(session),
	name_widget_(nullptr)
{
    ...
    connect(base_.get(), ScopySignal(enabled_changed(bool)),
		this, SLOT(on_enabled_changed(bool)));
}

void ScopySignal::set_name(QString name)
{
	base_->set_name(name);
...
bool ScopySignal::enabled() const
{
	return base_->enabled();
}

shared_ptr<data::ScopySignalBase> ScopySignal::base() const
{
	return base_;
}

void ScopySignal::save_settings(QSettings &settings) const
{
	(void)settings;
}

void ScopySignal::restore_settings(QSettings &settings)
{
	(void)settings;
}

void ScopySignal::paint_back(QPainter &p, ViewItemPaintParams &pp)
{
	if (base_->enabled())
		Trace::paint_back(p, pp);
}

void ScopySignal::populate_popup_form(QWidget *parent, QFormLayout *form)
{
    ...
    connect(name_widget_, ScopySignal(editTextChanged(const QString&)),
...
QMenu* ScopySignal::create_header_context_menu(QWidget *parent)
{
    ...
    connect(disable, ScopySignal(triggered()), this, SLOT(on_disable()));
	menu->addAction(disable);

	return menu;
}

void ScopySignal::delete_pressed()
{
	on_disable();
}

void ScopySignal::on_name_changed(const QString &text)
{
    // On startup, this event is fired when a session restores ScopySignal
...
void ScopySignal::on_disable()
{
	base_->set_enabled(false);
}

void ScopySignal::on_enabled_changed(bool enabled)
{
	(void)enabled;
