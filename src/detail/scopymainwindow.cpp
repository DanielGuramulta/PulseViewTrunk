...
const QString ScopyMainWindow::WindowTitle = tr("PulseView");

ScopyMainWindow::ScopyMainWindow(DeviceManager &device_manager, QWidget *parent) :
	QMainWindow(parent),
...
void ScopyMainWindow::show_session_error(const QString text, const QString info_text)
{
...
shared_ptr<views::ViewBase> ScopyMainWindow::get_active_view() const
{
...
shared_ptr<views::ViewBase> ScopyMainWindow::add_view(const QString &title,
...
void ScopyMainWindow::remove_view(shared_ptr<views::ViewBase> view)
{
...
shared_ptr<Session> ScopyMainWindow::add_session()
{
...
void ScopyMainWindow::remove_session(shared_ptr<Session> session)
{
...
void ScopyMainWindow::add_session_with_file(string open_file_name,
...
void ScopyMainWindow::add_default_session()
{
...
void ScopyMainWindow::save_sessions()
{
	QSettings settings;
	int id = 0;
...
void ScopyMainWindow::restore_sessions()
{
	QSettings settings;
	int i, session_count;
...
void ScopyMainWindow::on_setting_changed(const QString &key, const QVariant &value)
{
...
void ScopyMainWindow::save_ui_settings()
{
	QSettings settings;
...
void ScopyMainWindow::restore_ui_settings()
{
	QSettings settings;
...
shared_ptr<Session> ScopyMainWindow::get_tab_session(int index) const
{
...
QMenu* ScopyMainWindow::createPopupMenu()
{
	return nullptr;
}

bool ScopyMainWindow::restoreState(const QByteArray &state, int version)
{
	(void)state;
	(void)version;
...
void ScopyMainWindow::on_add_view(const QString &title, views::ViewType type,
	Session *session)
{
...
void ScopyMainWindow::on_focus_changed()
{
...
void ScopyMainWindow::on_focused_session_changed(shared_ptr<Session> session)
{
	last_focused_session_ = session;
...
void ScopyMainWindow::on_new_session_clicked()
{
	add_session();
}

void ScopyMainWindow::on_run_stop_clicked()
{
...
void ScopyMainWindow::on_settings_clicked()
{
	dialogs::Settings dlg(device_manager_);
	dlg.exec();
}

void ScopyMainWindow::on_session_name_changed()
{
...
void ScopyMainWindow::on_capture_state_changed(QObject *obj)
{
...
void ScopyMainWindow::on_new_view(Session *session)
{
	// We get a pointer and need a reference
	for (shared_ptr<Session>& s : sessions_)
		if (s.get() == session)
...
void ScopyMainWindow::on_view_close_clicked()
{
...
void ScopyMainWindow::on_tab_changed(int index)
{
...
void ScopyMainWindow::on_tab_close_requested(int index)
{
	shared_ptr<Session> session = get_tab_session(index);
...
void ScopyMainWindow::on_view_colored_bg_shortcut()
{
	GlobalSettings settings;
...
void ScopyMainWindow::on_view_sticky_scrolling_shortcut()
{
	GlobalSettings settings;
...
void ScopyMainWindow::on_view_show_sampling_points_shortcut()
{
	GlobalSettings settings;
...
void ScopyMainWindow::on_view_show_analog_minor_grid_shortcut()
{
	GlobalSettings settings;
...
void ScopyMainWindow::on_settingViewColoredBg_changed(const QVariant new_value)
{
...
void ScopyMainWindow::on_settingViewShowSamplingPoints_changed(const QVariant new_value)
{
	bool state = new_value.toBool();
...
void ScopyMainWindow::on_settingViewShowAnalogMinorGrid_changed(const QVariant new_value)
{
	bool state = new_value.toBool();
...
void ScopyMainWindow::on_close_current_tab()
{
