...
const float ScopyLogicSignal::Oversampling = 2.0f;

const QColor ScopyLogicSignal::EdgeColor(0x80, 0x80, 0x80);
const QColor ScopyLogicSignal::HighColor(0x00, 0xC0, 0x00);
const QColor ScopyLogicSignal::LowColor(0xC0, 0x00, 0x00);
const QColor ScopyLogicSignal::SamplingPointColor(0x77, 0x77, 0x77);

const QColor ScopyLogicSignal::SignalColors[10] = {
...
QColor ScopyLogicSignal::TriggerMarkerBackgroundColor = QColor(0xED, 0xD4, 0x00);
const int ScopyLogicSignal::TriggerMarkerPadding = 2;
const char* ScopyLogicSignal::TriggerMarkerIcons[8] = {
	nullptr,
...
QCache<QString, const QIcon> ScopyLogicSignal::icon_cache_;
QCache<QString, const QPixmap> ScopyLogicSignal::pixmap_cache_;

ScopyLogicSignal::ScopyLogicSignal(
    pv::ScopySession &session,
	shared_ptr<devices::Device> device,
	shared_ptr<data::SignalBase> base) :
    ScopySignal(session, base),
	device_(device),
...
shared_ptr<pv::data::SignalData> ScopyLogicSignal::data() const
{
	return base_->logic_data();
}

shared_ptr<pv::data::Logic> ScopyLogicSignal::logic_data() const
{
	return base_->logic_data();
}

void ScopyLogicSignal::save_settings(QSettings &settings) const
{
	settings.setValue("trace_height", signal_height_);
}

void ScopyLogicSignal::restore_settings(QSettings &settings)
{
...
pair<int, int> ScopyLogicSignal::v_extents() const
{
...
void ScopyLogicSignal::paint_mid(QPainter &p, ViewItemPaintParams &pp)
{
	QLineF *line;
...
void ScopyLogicSignal::paint_fore(QPainter &p, ViewItemPaintParams &pp)
{
	if (base_->enabled()) {
		if (trigger_match_) {
...
vector<LogicSegment::EdgePair> ScopyLogicSignal::get_nearest_level_changes(uint64_t sample_pos)
{
	assert(base_);
	assert(owner_);
...
void ScopyLogicSignal::paint_caps(QPainter &p, QLineF *const lines,
...
shared_ptr<pv::data::LogicSegment> ScopyLogicSignal::get_logic_segment_to_paint() const
{
...
void ScopyLogicSignal::init_trigger_actions(QWidget *parent)
{
...
const vector<int32_t> ScopyLogicSignal::get_trigger_types() const
{
...
QAction* ScopyLogicSignal::action_from_trigger_type(const TriggerMatchType *type)
{
	QAction *action;
...
const TriggerMatchType *ScopyLogicSignal::trigger_type_from_action(QAction *action)
{
...
void ScopyLogicSignal::populate_popup_form(QWidget *parent, QFormLayout *form)
{
    ScopySignal::populate_popup_form(parent, form);
...
void ScopyLogicSignal::modify_trigger()
{
...
const QIcon* ScopyLogicSignal::get_icon(const char *path)
{
...
const QPixmap* ScopyLogicSignal::get_pixmap(const char *path)
{
...
void ScopyLogicSignal::on_setting_changed(const QString &key, const QVariant &value)
{
    ScopySignal::on_setting_changed(key, value);
...
void ScopyLogicSignal::on_trigger()
{
	QAction *action;
...
void ScopyLogicSignal::on_signal_height_changed(int height)
{
