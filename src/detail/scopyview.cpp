/*
 * This file is part of the PulseScopyView project.
 *
...
namespace pv {
namespace ScopyViews {
namespace trace {

const Timestamp ScopyView::MaxScale("1e9");
const Timestamp ScopyView::MinScale("1e-12");

const int ScopyView::MaxScrollValue = INT_MAX / 2;

const int ScopyView::ScaleUnits[3] = {1, 2, 5};
...
bool CustomScrollArea::ScopyViewportEvent(QEvent *event)
{
	switch (event->type()) {
	case QEvent::Paint:
	...
	case QEvent::TouchEnd:
		return false;
	default:
        return QAbstractScrollArea::ScopyViewportEvent(event);
	}
}

ScopyView::ScopyView(ScopySession &session, bool is_main_View, QWidget *parent) :
    ViewBase(session, is_main_ScopyView, parent),

    // Note: Place defaults in ScopyView::reset_ScopyView_state(), not here
	...
	sticky_scrolling_(false)  // Default setting is set in MainWindow::setup_ui()
{
    ...
    ScopyViewport_ = new ScopyViewport(*this);
	scrollarea_ = new CustomScrollArea(splitter_);
    scrollarea_->setScopyViewport(ScopyViewport_);
    ...
    splitter_->setStretchFactor(1, 1);  // when the entire ScopyView is resized
	splitter_->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);

    ScopyViewport_->installEventFilter(this);
	ruler_->installEventFilter(this);
	header_->installEventFilter(this);

	// Set up settings and event handlers
	GlobalSettings settings;
    colored_bg_ = settings.value(GlobalSettings::Key_ScopyView_ColoredBG).toBool();
    snap_distance_ = settings.value(GlobalSettings::Key_ScopyView_SnapDistance).toInt();
...
void ScopyView::reset_ScopyView_state()
{
    ScopyViewBase::reset_ScopyView_state();
...
Session& ScopyView::session()
{
	return session_;
}

const Session& ScopyView::session() const
{
	return session_;
}

unordered_set< shared_ptr<ScopySignal> > ScopyView::signals() const
{
	return signals_;
}

void ScopyView::clear_signals()
{
    ScopyViewBase::clear_signalbases();
	signals_.clear();
}

void ScopyView::add_signal(const shared_ptr<Signal> signal)
{
    ScopyViewBase::add_signalbase(signal->base());
...
#ifdef ENABLE_DECODE
void ScopyView::clear_decode_signals()
{
	decode_traces_.clear();
}

void ScopyView::add_decode_signal(shared_ptr<data::DecodeSignal> signal)
{
...
void ScopyView::remove_decode_signal(shared_ptr<data::DecodeSignal> signal)
{
...
shared_ptr<Signal> ScopyView::get_signal_under_mouse_cursor() const
{
	return signal_under_mouse_cursor_;
}

ScopyView* ScopyView::ScopyView()
{
	return this;
}

const ScopyView* ScopyView::ScopyView() const
{
	return this;
}

ScopyViewport* ScopyView::ScopyViewport()
{
    return ScopyViewport_;
}

const ScopyViewport* ScopyView::ScopyViewport() const
{
    return ScopyViewport_;
}

const Ruler* ScopyView::ruler() const
{
	return ruler_;
}

void ScopyView::save_settings(QSettings &settings) const
{
...
void ScopyView::restore_settings(QSettings &settings)
{
	...
	if (settings.contains("ruler_shift")) {
		...
		try {
            ...
            qDebug() << "Could not restore the ScopyView ruler shift";
		}
	}
	...
	if (settings.contains("offset")) {
		...
		try {
            ...
            qDebug() << "Could not restore the ScopyView offset";
		}
	}
...
vector< shared_ptr<TimeItem> > ScopyView::time_items() const
{
...
double ScopyView::scale() const
{
	return scale_;
}

void ScopyView::set_scale(double scale)
{
	if (scale_ != scale) {
		scale_ = scale;
		scale_changed();
...
void ScopyView::set_offset(const pv::util::Timestamp& offset, bool force_update)
{
...
const Timestamp& ScopyView::offset() const
{
	return offset_;
}

const Timestamp& ScopyView::ruler_offset() const
{
	return ruler_offset_;
}

void ScopyView::set_zero_position(const pv::util::Timestamp& position)
{
...
void ScopyView::reset_zero_position()
{
	ruler_shift_ = 0;
...
int ScopyView::owner_visual_v_offset() const
{
	return -scrollarea_->verticalScrollBar()->sliderPosition();
}

void ScopyView::set_v_offset(int offset)
{
	scrollarea_->verticalScrollBar()->setSliderPosition(offset);
	header_->update();
    ScopyViewport_->update();
}

unsigned int ScopyView::depth() const
{
	return 0;
}

uint32_t ScopyView::current_segment() const
{
	return current_segment_;
}

pv::util::SIPrefix ScopyView::tick_prefix() const
{
	return tick_prefix_;
}

void ScopyView::set_tick_prefix(pv::util::SIPrefix tick_prefix)
{
...
unsigned int ScopyView::tick_precision() const
{
	return tick_precision_;
}

void ScopyView::set_tick_precision(unsigned tick_precision)
{
...
const pv::util::Timestamp& ScopyView::tick_period() const
{
	return tick_period_;
}

unsigned int ScopyView::minor_tick_count() const
{
	return minor_tick_count_;
}

void ScopyView::set_tick_period(const pv::util::Timestamp& tick_period)
{
...
TimeUnit ScopyView::time_unit() const
{
	return time_unit_;
}

void ScopyView::set_time_unit(pv::util::TimeUnit time_unit)
{
...
void ScopyView::set_current_segment(uint32_t segment_id)
{
	...
	GlobalSettings settings;
    bool trigger_is_zero_time = settings.value(GlobalSettings::Key_ScopyView_TriggerIsZeroTime).toBool();
...
bool ScopyView::segment_is_selectable() const
{
	return segment_selectable_;
}

Trace::SegmentDisplayMode ScopyView::segment_display_mode() const
{
	return segment_display_mode_;
}

void ScopyView::set_segment_display_mode(Trace::SegmentDisplayMode mode)
{
...
void ScopyView::zoom(double steps)
{
    zoom(steps, ScopyViewport_->width() / 2);
}

void ScopyView::zoom(double steps, int offset)
{
	set_zoom(scale_ * pow(3.0 / 2.0, -steps), offset);
}

void ScopyView::zoom_fit(bool gui_state)
{
    ...
    assert(ScopyViewport_);
    const int w = ScopyViewport_->width();
	if (w <= 0)
		return;
...
void ScopyView::set_scale_offset(double scale, const Timestamp& offset)
{
	// Disable sticky scrolling / always zoom to fit when acquisition runs
    // and user drags the ScopyViewport
...
set< shared_ptr<SignalData> > ScopyView::get_visible_data() const
{
...
pair<Timestamp, Timestamp> ScopyView::get_time_extents() const
{
...
void ScopyView::enable_show_sampling_points(bool state)
{
	(void)state;
...
void ScopyView::enable_show_analog_minor_grid(bool state)
{
	(void)state;
...
void ScopyView::enable_colored_bg(bool state)
{
	colored_bg_ = state;
    ScopyViewport_->update();
}

bool ScopyView::colored_bg() const
{
	return colored_bg_;
}

bool ScopyView::cursors_shown() const
{
	return show_cursors_;
}

void ScopyView::show_cursors(bool show)
{
	show_cursors_ = show;
...
void ScopyView::centre_cursors()
{
	if (cursors_) {
        const double time_width = scale_ * ScopyViewport_->width();
...
shared_ptr<CursorPair> ScopyView::cursors() const
{
	return cursors_;
}

void ScopyView::add_flag(const Timestamp& time)
{
...
void ScopyView::remove_flag(shared_ptr<Flag> flag)
{
	flags_.remove(flag);
	time_item_appearance_changed(true, true);
}

vector< shared_ptr<Flag> > ScopyView::flags() const
{
...
const QPoint& ScopyView::hover_point() const
{
	return hover_point_;
}

const QWidget* ScopyView::hover_widget() const
{
	return hover_widget_;
}

int64_t ScopyView::get_nearest_level_change(const QPoint &p)
{
...
void ScopyView::restack_all_trace_tree_items()
{
...
int ScopyView::header_width() const
{
	 return header_->extended_size_hint().width();
}

void ScopyView::on_setting_changed(const QString &key, const QVariant &value)
{
    if (key == GlobalSettings::Key_ScopyView_TriggerIsZeroTime)
        on_settingScopyViewTriggerIsZeroTime_changed(value);

    if (key == GlobalSettings::Key_ScopyView_SnapDistance) {
		GlobalSettings settings;
        snap_distance_ = settings.value(GlobalSettings::Key_ScopyView_SnapDistance).toInt();
	}
}

void ScopyView::trigger_event(int segment_id, util::Timestamp location)
{
	// TODO This doesn't work if we're showing multiple segments at once
	if ((uint32_t)segment_id != current_segment_)
		return;

    // Set zero location if the Key_ScopyView_TriggerIsZeroTime setting is set and
...
void ScopyView::get_scroll_layout(double &length, Timestamp &offset) const
{
	const pair<Timestamp, Timestamp> extents = get_time_extents();
...
void ScopyView::set_zoom(double scale, int offset)
{
...
void ScopyView::calculate_tick_spacing()
{
    ...
    const QSize areaSize = ScopyViewport_->size();
...
void ScopyView::adjust_top_margin()
{
    assert(ScopyViewport_);
...
void ScopyView::reset_scroll()
{
	scrollarea_->verticalScrollBar()->setRange(0, 0);
}

void ScopyView::set_scroll_default()
{
    assert(ScopyViewport_);
    ...
    // Do all traces fit in the ScopyView?
	if (areaSize.height() >= trace_height)
...
void ScopyView::determine_if_header_was_shrunk()
{
...
void ScopyView::resize_header_to_fit()
{
...
void ScopyView::update_layout()
{
	update_scroll();
}

TraceTreeItemOwner* ScopyView::find_prevalent_trace_group(
...
vector< shared_ptr<Trace> > ScopyView::extract_new_traces_for_channels(
...
void ScopyView::determine_time_unit()
{
...
bool ScopyView::eventFilter(QObject *object, QEvent *event)
{
	...
	if (type == QEvent::MouseMove) {
        ...
        // set_v_offset() from within restore_settings() as the ScopyView
        ...
        // the ScopyView is shown, which happens after all widgets were
...
void ScopyView::contextMenuEvent(QContextMenuEvent *event)
{
	QPoint pos = event->pos() - QPoint(0, ruler_->sizeHint().height());

    const shared_ptr<ScopyViewItem> r = ScopyViewport_->get_mouse_over_item(pos);
	if (!r)
		return;

    QMenu *menu = r->create_ScopyView_context_menu(this, pos);
	if (menu)
		menu->popup(event->globalPos());
}

void ScopyView::resizeEvent(QResizeEvent* event)
{
...
void ScopyView::update_hover_point()
{
...
void ScopyView::row_item_appearance_changed(bool label, bool content)
{
	if (label)
		header_->update();
	if (content)
...
void ScopyView::time_item_appearance_changed(bool label, bool content)
{
	if (label) {
		ruler_->update();
...
void ScopyView::extents_changed(bool horz, bool vert)
{
	sticky_events_ |=
...
void ScopyView::on_signal_name_changed()
{
	if (!header_was_shrunk_)
		resize_header_to_fit();
}

void ScopyView::on_splitter_moved()
{
...
void ScopyView::h_scroll_value_changed(int value)
{
	if (updating_scroll_)
		return;
...
void ScopyView::v_scroll_value_changed()
{
	header_->update();
    ScopyViewport_->update();
}

void ScopyView::signals_changed()
{
    ...
    // Make a look-up table of sigrok Channels to pulseScopyView Signals
...
void ScopyView::capture_state_updated(int state)
{
	GlobalSettings settings;
	...
	if (state == Session::Running) {
        ...
        // the main ScopyView of this session (other trace ScopyViews may be used for
		// zooming and we don't want to mess them up)
        bool state = settings.value(GlobalSettings::Key_ScopyView_ZoomToFitDuringAcq).toBool();
        if (is_main_ScopyView_ && state) {
			always_zoom_to_fit_ = true;
			always_zoom_to_fit_changed(always_zoom_to_fit_);
		}

		// Enable sticky scrolling if the setting is enabled
        sticky_scrolling_ = settings.value(GlobalSettings::Key_ScopyView_StickyScrolling).toBool();
	...
	if (state == Session::Stopped) {
		...
		bool zoom_to_fit_after_acq =
            settings.value(GlobalSettings::Key_ScopyView_ZoomToFitAfterAcq).toBool();

        // Only perform zoom-to-fit if the user hasn't altered the ScopyViewport and
...
void ScopyView::on_new_segment(int new_segment_id)
{
	on_segment_changed(new_segment_id);
}

void ScopyView::on_segment_completed(int segment_id)
{
	on_segment_changed(segment_id);
}

void ScopyView::on_segment_changed(int segment)
{
...
void ScopyView::on_settingScopyViewTriggerIsZeroTime_changed(const QVariant new_value)
{
	if (new_value.toBool()) {
...
void ScopyView::perform_delayed_ScopyView_update()
{
	if (always_zoom_to_fit_) {
		zoom_fit(true);
	} else if (sticky_scrolling_) {
        // Make right side of the ScopyView sticky
		double length = 0;
		Timestamp offset;
		get_scroll_layout(length, offset);
...
void ScopyView::process_sticky_events()
{
